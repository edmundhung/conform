...
test.describe('shadcn-ui', () => {
	...
	test('blur', async ({ page }) => {
		...
		await form.accountType.getByRole('radio').last().click();
		await form.accountType.getByRole('radio').last().click();
		await expect(
			form.accountType.getByRole('radio').last(),
		).toHaveAccessibleDescription('');
		await form.heading.click();
		await expect(
			form.accountType.getByRole('radio').last(),
		...
		await form.categories.getByRole('button').last().click();
		await form.categories.getByRole('button').first().click();
		await form.categories.getByRole('button').last().click();
		await expect(
