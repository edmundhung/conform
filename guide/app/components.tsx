...
export function Row({ children }: { children: React.ReactNode }) {
	return (
		<div
			className={`
				grid grid-cols-1 items-start gap-x-16 gap-y-10 xl:max-w-none xl:grid-cols-2 my-16
			`}
		>
			{children}
		</div>
	...
	return (
		<div className="group dark:bg-white/2.5">
			...
			<div className="relative">
				<pre className="overflow-x-auto text-xs text-white">{children}</pre>
...
function usePreventLayoutShift() {
	let positionRef = useRef<HTMLDivElement>(null);
	...
	return {
		...
		preventLayoutShift(callback: () => void) {
			...
			rafRef.current = window.requestAnimationFrame(() => {
				...
				if (
					typeof newTop !== 'undefined' &&
					typeof initialTop !== 'undefined'
				) {
...
function useTabGroupProps(availableLanguages: string[]) {
	...
	return {
		...
		onChange: (newSelectedIndex: number) => {
			preventLayoutShift(() => setSelectedIndex(newSelectedIndex));
		},
	};
}
