...
describe('coercion', () => {
	describe('coerceFormValue', () => {
		test('recursive schema via z.lazy', () => {
			type Tree = {
				value: string;
				children: Tree[];
			};
			...
			expect(
				...
				success: false,
				error: {
					'tree.value': expect.any(Array),
					'tree.children[0].value': expect.any(Array),
				},
			});
		});
	});
	...
	describe('coerceStructure', () => {
		...
		test('error', () => {
			...
			if (!dateResult.success) {
				expect(dateResult.error.issues[0]).toMatchObject({
					code: 'invalid_type',
					expected: 'date',
					received: 'number',
					path: [],
		...
		test('discriminated union', () => {
			...
			expect(schema.parse({ type: 'number', value: 'abc' })).toEqual({
				type: 'number',
				value: NaN,
			});

			// invalid discriminator
			expect(() => schema.parse({ type: 'bogus', value: '' })).toThrow();
