 ...
 * Used by useFormData and useControl to subscribe to DOM events.
 */
const globalObserver = createGlobalFormsObserver();
...
export function useControl(options?: {
	...
	const snapshot = useSyncExternalStore(
		useCallback(
			(callback) =>
				globalObserver.onFieldUpdate((event) => {
	...
	const value = useSyncExternalStore(
		useCallback(
			(callback) => {
				...
				const unsubscribe = globalObserver.onFormUpdate((event) => {
...
>(
	...
	const globalConfig = {
		...
		schema: config.schema ?? standardSchema,
	};
	 ...
	 */
	function useForm<
		Schema extends ExtractSchemaType<TypeKey>,
		ErrorShape extends BaseErrorShape = BaseErrorShape,
		Value = InferSchemaOutput<TypeKey, Schema>,
	>(
		schema: Schema,
		options: FormOptions<
			InferSchemaInput<TypeKey, Schema> extends Record<string, any>
	...
	function useForm<
		Schema extends ExtractSchemaType<TypeKey> &
			StandardSchemaV1<any, any> = ExtractSchemaType<TypeKey> &
	...
	>(
		...
		>(formId, {
			...
			onValidate(ctx) {
				if (schema) {
					const schemaResult = globalConfig.schema.validate(
						schema,
						ctx.payload,
					);
					...
					const schemaValue = schemaResult.value as InferSchemaOutput<
						TypeKey,
						Schema
					>;
